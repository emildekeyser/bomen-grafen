...
public class Graph
{
	...
	{	
		// nummering van start-knoop (1..aantal_knopen) naar eindKnoop (destination)
		...
		while (!queue.isEmpty() && ancestors[destination-1] == infty)
		{
			for (int x = 0; x < this.getAantalKnopen(); x++)
			{
				if (this.verbindingsMatrix[queue.peek()-1][x] && ancestors[x] == infty)
				{		
					ancestors[x] = queue.peek();
					queue.add(x+1);
				}
			}
	...
	public List<Integer> findPath(int start, int destination)
	{
		...
		{
			path.add(0, ancestors[next-1]);
			next = ancestors[next-1];
