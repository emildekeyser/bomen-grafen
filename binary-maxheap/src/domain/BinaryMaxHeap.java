...
public class BinaryMaxHeap<E extends Comparable<E>> implements Iterable<E>
{
	...
	public E getMin()
	{
		...
		{
			double lastLevel = BinaryMaxHeap.log2(this.values.size()-1);
			lastLevel = Math.floor(lastLevel);
			E lowest = this.values.get((int) lastLevel);
			
			for (int i = (int)lastLevel+1; i < this.values.size(); i++)
			{
				if (this.values.get(i).compareTo(lowest) < 0)
				{
					lowest = this.values.get(i);
				}
			}
	...
	private void bubbleUp()
	{
		...
		while (!ready)
		{
			if (child % 2 == 0) // even dus rechter kind (wanterig root0 + kind1
								// => klein boompje kind1 is links, kind2 zou
								// rechts zijn)
			{
	...
	public E removeLargest()
	{
		...
		{
			E temp = this.getMax();// res bevat de kleinste = eerste element van
									// de lijst
			this.values.set(0, this.values.get(this.values.size() - 1));// verwissel
																		// eerste
																		// met
																		// de
																		// laatste
			this.values.remove(this.values.size() - 1); // verwijder de laatste
			this.bubbleDown(); // bubble down van eerste naar beneden zie
								// theorie
			return temp;
		}
	...
	private void bubbleDown()
	{
		...
		while (!ready)
		{
			...
			valueright = childright < this.values.size()
					? this.values.get(childright)
					: null;
			valueleft = childleft < this.values.size()
					? this.values.get(childleft)
					: null;
			...
			{
				if (valueparent.compareTo(valueleft) < 0
						|| valueparent.compareTo(valueright) < 0)
				{
